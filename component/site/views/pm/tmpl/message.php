...
if ($this->message) {
	...
	echo '<a href="'.JRoute::_("index.php?option=com_mue&view=pm&layout=replymessage&mid=".$this->message->msg_id).'" class="button uk-button uk-button-primary">Reply</a> ';
	echo '<a href="'.JRoute::_("index.php?option=com_mue&view=pm&layout=messages").'" class="button uk-button uk-button-default">Message List</a> ';
	echo '<a href="'.JRoute::_("index.php?option=com_mue&view=pm&layout=trashmessage&mid=".$this->message->msg_id).'" class="button uk-button uk-button-danger">Trash</a> ';
	echo '<a href="'.JRoute::_("index.php?option=com_mue&view=pm&layout=spammessage&mid=".$this->message->msg_id).'" class="button uk-button uk-button-danger">Report as SPAM</a> ';
	echo '</p>';
	...
	echo '<div class="uk-form-row uk-margin-top mue-user-info-row mue-row1"><div class="uk-form-label mue-user-info-label uk-text-bold">From</div><div class="uk-form-controls uk-form-controls-text mue-user-info-hdr">'.$this->message->name.'</div></div>';

		...
		<?php foreach($this->items as $i => $item): ?>
			<tr class="row<?php echo $i % 2; ?>">
		        ...
		        <td class="order">	<div class="input-prepend">
						<?php if ($saveOrder) :?>
							<?php if ($listDirn == 'asc') : ?>
								<span class="add-on"><?php echo $this->pagination->orderUpIcon($i, ($item->opt_question == @$this->items[$i-1]->opt_question), 'ugroups.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								<span class="add-on"><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, ($item->opt_question == @$this->items[$i+1]->opt_question), 'ugroups.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php elseif ($listDirn == 'desc') : ?>
								<span class="add-on"><?php echo $this->pagination->orderUpIcon($i, ($item->opt_question == @$this->items[$i-1]->opt_question), 'ugroups.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								<span class="add-on"><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, ($item->opt_question == @$this->items[$i+1]->opt_question), 'ugroups.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php endif; ?>
						<?php endif; ?>

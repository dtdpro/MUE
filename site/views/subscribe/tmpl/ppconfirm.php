...
<form action="<?php echo JRoute::_('index.php?option=com_mue&view=subscribe&layout=ppverify&purchaseid='.$this->usid.'&plan='.$this->pinfo->sub_id); ?>" method="POST">
			<p style="padding-left:10px;">Your payment has been verified, please press <b>Complete Payment</b> to finalize your payment and charge your account for the following plan:<br /><br />
			<?php 
			
			echo '<b>'.$this->pinfo->sub_exttitle.'</b><br />'.$this->pinfo->sub_desc; 
			$btntext = "Complete Payment $".$session->get('TotalAmount');
